...
​		就目前而言`前端脚手架`其实在各大企业已经是一个极为成熟的项目了，那么我们到底该如何应用一个企业出厂的脚手架呢？
...
​		类似在我们实际项目开发之处，往往需要配置许多环境以及一系列的工具，而这一合集就可以称为我么程序中的脚手架。
...
* [commander](https://www.npmjs.com/package/commander)：完整的 [node.js](http://nodejs.org/) 命令行解决方案
  ...
  ```shell
  lerna add commander packages/core --D
  ```

* [fs-extra](https://www.npmjs.com/package/fs-extra)： 强大的文件处理工具

* [chalk](https://www.npmjs.com/package/chalk) ：粉饰你的终端，给你的终端文字添加样式
...
* [npmlog](https://www.npmjs.com/package/npmlog) ：打印日志，（需在 packages/utils 中初始化）
  ...
  ```shell
  lerna add npmlog @types/npmlog packages/utils --D
  ```
...
​		其实还有许多可以扩展开发的地方，经过这一整个实现下来，在下面进行扩展其实都很简单了，也方便根据自身去进行二次开发了



​		源码地址 github，脚手架已发布至 npm